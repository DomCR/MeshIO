...
{
	public class ParserConverter7400 : BaseParserConverter
	{
		public ParserConverter7400(FbxRootNode root) : base(root) { }
	}

	public abstract class BaseParserConverter : IParserConverter
	{
		public static IParserConverter GetConverter(FbxRootNode root)
		{
			IParserConverter converter = null;
			...
			switch (root.Version)
			{
				...
				case FbxVersion.v7400:
					converter = new ParserConverter7400(root);
					break;
		...
		public Dictionary<ulong, FbxNode> Objects = new Dictionary<ulong, FbxNode>();

		protected readonly FbxRootNode _root;

		public BaseParserConverter(FbxRootNode root)
		{
			_root = root;

			foreach (FbxNode n in getObjects().Nodes)
			{
				Objects.Add(Convert.ToUInt64(n.Properties[0]), n);
			}
		}
		...
		protected IEnumerable<FbxNode> getChildren(ulong containerId)
		{
			foreach (FbxNode c in getConnections().Nodes)
			{
				if (Convert.ToUInt64(c.Properties[2]) == containerId)
				{
					...
					yield return this.Objects[elementId];
				}
			}
		}
	}
