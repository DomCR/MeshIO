...
{
	/// <summary>
	/// Converts a <see cref="Scene"/> to a <see cref="FbxRootNode"/>
	/// </summary>
	public abstract class SceneConverterBase : ISceneConverter
	{
		...
		public static ISceneConverter GetConverter(Scene scene, FbxVersion version)
		{
			ISceneConverter converter = null;
			...
			switch (version)
			{
				...
				case FbxVersion.v7700:
					converter = new SceneConverter7000(scene);
					break;
				default:
					throw new NotSupportedException($"Incompatible version {version}");
			}
		...
		protected SceneConverterBase(Scene scene, FbxVersion version)
		{
			this._scene = scene;
		...
		protected void buildObjectsNode()
		{
			...
			foreach (Element3D item in this._scene.RootNode.Children)
			{
				...
				addConnection("OO", item.Id.Value, 0);
			}
		}

		protected FbxNode buildElementNode(Element3D element)
		{
			FbxNode node = null;
		...
		protected FbxNode buildModel(Node n, FbxNode properties)
		{
			...
			foreach (Element3D item in n.Children)
			{
				...
				addConnection("OO", item.Id.Value, n.Id.Value);
			}
		...
		{
			FbxNode node = new FbxNode("Material", n.Id, $"Material::{n.Name}", "Null");
