	...
	where T : Geometry
{
	...
	protected override bool setValue(FbxFileBuilderBase builder, FbxNode node)
	{
		switch (node.Name)
		{
			case FbxFileToken.Layer:
				return true;
			case FbxFileToken.LayerElementNormal:
				this._element.Layers.Add(mapLayerElementNormal(builder.Version, node));
				return true;
			case FbxFileToken.LayerElementBinormal:
				this._element.Layers.Add(mapLayerElementBinormal(builder.Version, node));
				return true;
			case FbxFileToken.LayerElementTangent:
				this._element.Layers.Add(mapLayerElementTangent(builder.Version, node));
				return true;
			case FbxFileToken.LayerElementMaterial:
				this._element.Layers.Add(BuildLayerElementMaterial(builder.Version, node));
				return true;
			case FbxFileToken.LayerElementUV:
				this._element.Layers.Add(mapLayerElementUV(builder.Version, node));
				return true;
			case FbxFileToken.LayerElementSmoothing:
				this._element.Layers.Add(mapLayerElementSmoothing(builder.Version, node));
				return true;
			default:
